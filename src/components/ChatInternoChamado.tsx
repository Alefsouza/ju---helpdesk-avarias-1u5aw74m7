  ...
  const fetchMensagens = useCallback(async () => {
    ...
    if (userIds.length > 0) {
      const { data: perfis } = await supabase
        ...
        .in('id', userIds)
      perfis?.forEach((p) => {
        perfilMap[p.id] = { nome_completo: p.nome_completo, email: p.email }
      })
    }

    const mapped: MensagemInterna[] = (data || []).map((m: MensagemInterna) => ({
      ...m,
      usuario: perfilMap[m.usuario_id] || null,
    }))
      ...
      .on(
        ...
        async (payload) => {
          ...
          setMensagens((prev) => {
            ...
            return [...prev, { ...nova, usuario: perfil || null }]
          })
        },
      )
      .on(
                ...
                >
                  <div
                    className={cn(
                      ...
                      isCurrentUser
                        ? 'bg-amber-500 text-white rounded-tr-sm border-amber-600'
                        : 'bg-white text-slate-800 rounded-tl-sm border-amber-200',
                    )}
                  >
                    {!isCurrentUser && (
                      <div className="font-bold text-[11px] mb-0.5 text-amber-700">
                        {m.usuario?.nome_completo || 'Usuário'}
                      </div>
                    )}
