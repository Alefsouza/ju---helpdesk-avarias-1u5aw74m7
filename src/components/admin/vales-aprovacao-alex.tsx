...
export function ValesAprovacaoAlex() {
  ...
  const fetchChamados = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `id, titulo, criado_em, atualizado_em, responsavel_id, usuario_id, status_aprovacao_alex, status_interno,
         anexos_chamado_interno ( id, nome_arquivo )`,
      )
    ...
    const userIds = [...new Set(filteredData.map((c) => c.usuario_id).filter(Boolean))]
  ...
  const handleRejectSubmit = async () => {
    if (!selectedChamado || !rejectReason.trim()) {
      toast.error('Informe a observação da desaprovação')
      return
    }
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          status_aprovacao_alex: 'reprovado',
      ...
      toast.success('Chamado desaprovado e reaberto com sucesso!')
      setIsRejectOpen(false)
      fetchChamados()
    } catch (error: any) {
      toast.error('Erro ao desaprovar chamado: ' + error.message)
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <Card>
      ...
      <Dialog open={isRejectOpen} onOpenChange={setIsRejectOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Motivo da Desaprovação</DialogTitle>
            <DialogDescription>
              Informe o motivo da desaprovação. O chamado será reaberto e o colaborador será
              notificado.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Textarea
                id="rejectReasonAlex"
                placeholder="Descreva o motivo da desaprovação..."
                value={rejectReason}
