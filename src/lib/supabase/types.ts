...
//   CHECK documentos_tipo_documento_check: CHECK ((tipo_documento = ANY (ARRAY['IDO'::text, 'Espelho de Danos'::text, 'Vistoria'::text, 'Boletim de Ocorrência'::text, 'Apólice'::text, 'Valor do acordo'::text, 'Relato manuscrito'::text, 'OS de Manutenção'::text, 'Orçamento'::text, 'Vale'::text, 'CNH'::text, 'Documento do veículo'::text, 'Fotos/Vídeos'::text, 'Anexo Lesão Corporal'::text, 'Outros'::text])))
// Table: formularios_espelho_danos
...
// Table: rascunhos_chamado
//   PRIMARY KEY rascunhos_chamado_pkey: PRIMARY KEY (id)
//   FOREIGN KEY rascunhos_chamado_usuario_id_fkey: FOREIGN KEY (usuario_id) REFERENCES auth.users(id) ON DELETE CASCADE
//   UNIQUE rascunhos_chamado_usuario_id_key: UNIQUE (usuario_id)
// Table: respostas_chamado
...
//     USING: ((usuario_id = auth.uid()) OR (responsavel_id = auth.uid()) OR is_admin() OR is_sos() OR is_coc() OR (((status = 'aberto'::text) OR (status = 'finalizado'::text)) AND (is_responsavel() OR (is_sinistro() AND (( SELECT perfil_usuario.garagem    FROM perfil_usuario   WHERE (perfil_usuario.id = auth.uid())) = COALESCE(garagem, ( SELECT perfil_usuario.garagem    FROM perfil_usuario   WHERE (perfil_usuario.id = chamados.usuario_id))))))) OR (id IN ( SELECT participantes_chamado.chamado_id    FROM participantes_chamado   WHERE (participantes_chamado.usuario_id = auth.uid()))))
...
// Table: rascunhos_chamado
//   Policy "rascunhos_chamado_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (usuario_id = auth.uid())
//     WITH CHECK: (usuario_id = auth.uid())
// Table: respostas_chamado
...
//   $function$
//
// FUNCTION is_restricted_user(uuid)
