...
export default function SinistrosCoc() {
  ...
  const uniqueMonths = useMemo(() => {
    const months = new Set(
      sinistros.map(s => format(parseISO(s.criado_em), 'yyyy-MM'))
    )
  ...
  const filteredSinistros = useMemo(() => {
    return sinistros.filter(s => {
      if (monthFilter !== 'all' && format(parseISO(s.criado_em), 'yyyy-MM') !== monthFilter) return false
              ...
              {loading ? (
                ...
                filteredSinistros.map(s => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell className="whitespace-nowrap">
                      <Badge variant="outline" className={s.tipo_chamado === 'Avaria com vítima' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-orange-50 text-orange-600 border-orange-200'}>
                        {s.tipo_chamado || '-'}
                      </Badge>
                    </TableCell>
                    <TableCell className="whitespace-nowrap">
                      {s.numero_os ? (
                        ...
                        <Badge variant="secondary" className="text-slate-500">Sem OS</Badge>
                      )}
                    </TableCell>
                    <TableCell className="text-right whitespace-nowrap">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => openView(s)} title="Visualizar">
                          <Eye className="w-4 h-4 text-slate-600" />
                        </Button>
                        {!s.numero_os && (
                          <Button variant="ghost" size="icon" onClick={() => openOS(s)} title="Preencher OS">
          ...
          {selectedSinistro && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm">{format(parseISO(selectedSinistro.criado_em), 'dd/MM/yyyy HH:mm')}</p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm">{selectedSinistro.nome_motorista || '-'} (Reg: {selectedSinistro.registro_motorista || '-'})</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm font-medium text-slate-500">Colaborador</p>
                <p className="text-sm">{selectedSinistro.nome_cobrador || '-'} (Reg: {selectedSinistro.registro_cobrador || '-'})</p>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1 sm:col-span-2">
                ...
                <p className="text-sm font-bold text-emerald-600">{selectedSinistro.numero_os || 'Não preenchido'}</p>
              </div>
              ...
              {selectedSinistro.anexos_chamado?.[0] && (
                <div className="space-y-1 sm:col-span-2 mt-4">
                  ...
                  {selectedSinistro.anexos_chamado[0].url_arquivo.toLowerCase().match(/\.(jpeg|jpg|gif|png)$/) ? (
                    <img 
      ...
      <Dialog open={osModalOpen} onOpenChange={setOsModalOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Vincule um número de OS a este sinistro.
            </DialogDescription>
          </DialogHeader>
          
          <div className="py-4">
            <div className="space-y-2">
              <Label htmlFor="os_number">Número da OS <span className="text-red-500">*</span></Label>
              <Input
                id="os_number"
