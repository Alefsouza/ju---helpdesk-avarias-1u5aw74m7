...
export default function Defiridos() {
  const { user } = useAuth()
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem('defiridos_col_widths')
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      setColumnWidths((prev) => {
        ...
        localStorage.setItem('defiridos_col_widths', JSON.stringify(updated))
        return updated
      })
    }
