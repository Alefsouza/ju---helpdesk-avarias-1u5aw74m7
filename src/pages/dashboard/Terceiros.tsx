...
export default function Terceiros() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem('terceiros_col_widths')
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      setColumnWidths((prev) => {
        ...
        localStorage.setItem('terceiros_col_widths', JSON.stringify(updated))
        return updated
      })
    }
  ...
  const fetchChamados = async () => {
    ...
    setError(false)
    try {
      // 1. Buscar a garagem do usuário em perfil_usuario
      const { data: perfilData, error: perfilError } = await supabase
        .from('perfil_usuario')
        .select('garagem, tipo_usuario')
        .eq('id', user.id)
        .single()

      if (perfilError) throw perfilError

      const userGaragem = perfilData?.garagem?.trim() || null

      // 2. Buscar chamados com status_sinistro = 'Terceiros'
      let query = (supabase.from('chamados') as any)
        .select('*')
        .eq('status_sinistro', 'Terceiros')
      ...
      const { data: rawData, error: err } = await query

      if (err) throw err

      if (rawData && rawData.length > 0) {
        // Obter prefixos únicos dos carros para cruzar com frota_veiculos
        const prefixos: string[] = Array.from(
          new Set(rawData.map((c: any) => c.carro?.trim()).filter((p): p is string => Boolean(p))),
        )

        let frotaMap: Record<string, string> = {}
        if (prefixos.length > 0) {
          const { data: frotaData } = await (supabase.from('frota_veiculos') as any)
            .select('prefixo, garagem')
            .in('prefixo', prefixos)

          if (frotaData) {
            frotaMap = (frotaData as any[]).reduce(
              (acc, f) => {
                acc[(f.prefixo || '').trim().toLowerCase()] = (f.garagem || '').trim()
                return acc
              },
        ...
        // 3. Filtrar chamados cuja garagem do veículo coincide com a garagem do usuário
        const filteredByGaragem = rawData.filter((c: any) => {
          if (!userGaragem) return false
          const carroPrefixo = (c.carro || '').trim().toLowerCase()
          const veiculoGaragem = frotaMap[carroPrefixo] || (c.garagem || '').trim()
          return veiculoGaragem.toLowerCase() === userGaragem.toLowerCase()
        })

        if (filteredByGaragem.length > 0) {
          const userIds: string[] = Array.from(
            new Set(
              filteredByGaragem
                .flatMap((c: any) => [c.usuario_id, c.responsavel_id])
                .filter((id): id is string => Boolean(id)),
            ),
          )
          const { data: perfis } = await (supabase.from('perfil_usuario') as any)
          ...
          const chamadosComNome = filteredByGaragem.map((c) => ({
            ...c,
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-2 sm:p-4 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Terceiros</h1>
          <p className="text-slate-500">Chamados classificados como Terceiros.</p>
        </div>
      </div>
