  ...
  try {
    ...
    if (tipo_documento === 'Vale') {
      ...
      if (tipo_documento === 'IDO') {
        ...
        let dataOcorrenciaFormatada = '-'
        if (data) {
          const [yyyy, mm, dd] = data.split('-')
          if (yyyy && mm && dd) {
            dataOcorrenciaFormatada = `${dd}/${mm}/${yyyy}`
          } else {
            dataOcorrenciaFormatada = data
          }
        }
        drawText(`Data da Ocorrência: ${dataOcorrenciaFormatada}`)
        drawText(`Horário da Ocorrência: ${horario || '-'}`)
        drawText(
