-- Allow Financeiro and Contabil users to SELECT chamados
...
CREATE POLICY "chamados_select" ON public.chamados
  ...
  USING (
    ...
    (id IN ( SELECT participantes_chamado.chamado_id FROM participantes_chamado WHERE (participantes_chamado.usuario_id = auth.uid()))) OR
    ((SELECT tipo_usuario FROM public.perfil_usuario WHERE id = auth.uid()) = 'financeiro') OR
    ((SELECT tipo_usuario FROM public.perfil_usuario WHERE id = auth.uid()) = 'contabil')
  );
